...
kernel void yoshida_no_planets(global float2 *positions,
                               global float2 *velocities, const float dt,
                               const float2 global_gravity) {
  ...
  const float2 v1 = v0 + global_gravity * (float2)(D1 * dt, D1 * dt);
  const float2 x2 = x0 + v1 * (float2)(C2 * dt, C2 * dt);
  const float2 v2 = v0 + global_gravity * (float2)(D2 * dt, D2 * dt);
  const float2 x3 = x0 + v2 * (float2)(C3 * dt, C3 * dt);
  const float2 v3 = v0 + global_gravity * (float2)(D3 * dt, D3 * dt);
...
kernel void yoshida(global float2 *positions, global float2 *velocities,
                    const float dt, const float2 global_gravity,
