...
kernel void leapfrog_yoshida(
    global float2 *restrict positions,
    ...
    const float2 x2 = fma(v1, C2 * dt, x0);
    ...
    const float2 v2 = fma(a2, D2 * dt, v0);
    const float2 x3 = fma(v2, C3 * dt, x0);
